...
class Image extends React.Component {
    ...
    async loadImage() {
        ...
        const useImage = findImageIndex(this.props.event);

        if (this.props.event && this.props.event.images && this.props.event.images[useImage] && this.props.event.images[useImage].id) {
            let URI = `http://dev.dragonflyathletics.com:1337/api/dfkey/events/${this.props.event.id}/media/${this.props.event.images[useImage].id}`;
            ...
            fetch(URI, options)
                .then(response => {
                    ...
                    } else if (response.status > 200 && response.status < 300 || response.status === 200 && contentLength < 1000) {
                        this.loadImage();
                        return null;
                    } else {
